...
class Registry:
    ...
    def push(
        ...
        for blob in files:
            ...
            blob_to_use_for_layer = oras.utils.make_targz(blob) if is_dir_layer else blob
            # Create a new layer from the blob
            layer = oras.oci.NewLayer(blob_to_use_for_layer, is_dir=is_dir_layer, media_type=media_type)
            annotations = annotset.get_annotations(blob)

            # Always strip blob_name of path separator
            layer["annotations"] = {
                oras.defaults.annotation_title: blob.strip(os.sep)
            }
    ...
    def pull(
        ...
        for layer in manifest.get("layers", []):
            annotations = layer.get("annotation", {})
            ...
            unpack_layer = annotations.get(
                oras.defaults.annotation_unpack,
                False
            )
